...
PropertyTab syntax_highlighting_tab {
    "Syntax Highlighting",
    {
...
void MainWidget::add_property_tab(PropertyTab& property_tab)
{
    ...
    for (auto const& property : property_tab.properties) {
        ...
        // FIXME: This is a hack, because the layout calculates it as having 0 size without it.
        ...
        property.role.visit(
            [&](Gfx::AlignmentRole role) {
                ...
                alignment_picker.set_selected_index((size_t)(m_preview_widget->preview_palette().alignment(role), GUI::AllowCallback::No));
...
#define __ENUMERATE_ALIGNMENT_ROLE(role)                                                    \
    if (auto alignment_input = m_alignment_inputs[to_underlying(Gfx::AlignmentRole::role)]) \
        alignment_input->set_selected_index((size_t)(m_preview_widget->preview_palette().alignment(Gfx::AlignmentRole::role), GUI::AllowCallback::No));
    ENUMERATE_ALIGNMENT_ROLES(__ENUMERATE_ALIGNMENT_ROLE)
#undef __ENUMERATE_ALIGNMENT_ROLE
